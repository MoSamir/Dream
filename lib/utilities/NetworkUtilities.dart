...
import 'package:dream_soft/data_providers/models/ResponseViewModel.dart';
...
class NetworkUtilities {
  ...
  static Future<ResponseViewModel<dynamic>> handleGetRequest(
    ...
    try {
      var serverResponse = await http.get(methodURL, headers: requestHeaders).timeout(Duration(seconds: 30),onTimeout: (){
      ...
      getResponse = ResponseViewModel(
        isSuccess: false,
        errorViewModel: Constants.connectionTimeOut,
        responseData: null,
      );
    } catch (exception) {
  ...
  static Future<ResponseViewModel> handlePostRequest(
    ...
    try {
      http.Response serverResponse = await http.post(methodURL,
          headers: requestHeaders,
          body: acceptJson ? json.encode(requestBody) : requestBody).timeout(Duration(seconds: 30), onTimeout: (){
            throw SocketException;
      });
      if (serverResponse.statusCode == 200) {
